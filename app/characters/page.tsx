...
export default function Characters() {
  ...
  async function generateCharacterImage() {
    ...
    try {
      const res = await fetch('/api/generate-character', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ description }) })
  ...
  async function pollTraining(requestId: string, characterId: string) {
    const interval = setInterval(async () => {
      const res = await fetch('/api/check-training', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requestId, characterId }) })
      const data = await res.json()
      if (data.status === 'COMPLETED') { clearInterval(interval); await loadCharacters() }
    }, 30000)
  }
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-[#e8e8e8] font-sans">
      <header className="border-b border-[#222] px-8 py-5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-[#555] tracking-widest uppercase">角色庫</span>
        </div>
        <nav className="flex gap-2">
          {[
            ...
            <a key={nav.href} href={nav.href} className="px-3 py-1.5 rounded-full text-xs border border-[#222] text-[#555] hover:text-[#e8d5b0] hover:border-[#e8d5b0] transition-all">{nav.label}</a>
          ))}
          <span className="px-3 py-1.5 rounded-full text-xs bg-[#e8d5b0] text-[#0a0a0a] font-bold">👤 角色庫</span>
        </nav>
      </header>
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-serif text-[#e8d5b0] italic">角色庫</h1>
            <p className="text-xs text-[#555] mt-1">儲存角色參考圖，生成影片時保持角色一致性</p>
          </div>
          <button onClick={() => setShowAdd(!showAdd)} className="px-5 py-2.5 bg-[#e8d5b0] text-[#0a0a0a] rounded-xl text-xs font-bold tracking-widest uppercase hover:opacity-90 transition-all">+ 新增角色</button>
        </div>
        {showAdd && (
          <div className="bg-[#111] border border-[#e8d5b0]/30 rounded-2xl p-6 mb-8">
            ...
            {error && <div className="bg-red-900/20 border border-red-800/40 rounded-xl px-4 py-3 text-sm text-red-400 mb-4">{error}</div>}
            <div className="grid grid-cols-2 gap-6">
              <div className="flex flex-col gap-4">
                <div>
                  <div className="text-[10px] font-bold tracking-widest uppercase text-[#555] mb-2">角色名字</div>
                  <input value={name} onChange={e => setName(e.target.value)} placeholder="例如：Mia、阿俊" className="w-full bg-[#0a0a0a] border border-[#222] rounded-lg px-3 py-2.5 text-sm text-[#e8e8e8] outline-none focus:border-[#e8d5b0] transition-colors" />
                </div>
                <div>
                ...
                <div className="flex gap-3">
                  ...
                  <label className="flex-1 py-2.5 border border-[#222] rounded-xl text-xs font-bold tracking-widest uppercase text-[#555] hover:border-[#e8d5b0] hover:text-[#e8d5b0] transition-all cursor-pointer text-center">📁 上傳圖片<input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" /></label>
                </div>
              </div>
              <div className="flex flex-col gap-4">
                ...
                <div className="flex-1 bg-[#0a0a0a] border border-[#222] rounded-xl overflow-hidden flex items-center justify-center min-h-[250px]">
                  {uploadedImage ? <img src={uploadedImage} alt="preview" className="w-full h-full object-cover" /> : <div className="text-[#333] text-xs text-center"><div className="text-3xl mb-2">👤</div>AI 生成或上傳圖片（可選）</div>}
                </div>
                <button onClick={addCharacter} className="w-full py-3 bg-[#e8d5b0] text-[#0a0a0a] rounded-xl text-xs font-bold tracking-widest uppercase hover:opacity-90 transition-all">儲存角色</button>
              </div>
            </div>
          </div>
        )}
        {loading ? (
          <div className="flex items-center justify-center min-h-[400px]"><div className="text-[#555] text-sm">載入中...</div></div>
        ) : characters.length === 0 ? (
            ...
            {characters.map(char => (
              <div key={char.id} className="bg-[#111] border border-[#222] rounded-2xl overflow-hidden group">
                <div className="aspect-[9/16] overflow-hidden relative bg-[#0a0a0a] flex items-center justify-center">
                  {char.imageUrl ? <img src={char.imageUrl} alt={char.name} className="w-full h-full object-cover" /> : <div className="text-4xl opacity-20">👤</div>}
                  <button onClick={() => deleteCharacter(char.id)} className="absolute top-3 right-3 w-8 h-8 bg-black/60 rounded-full text-xs text-red-400 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center">✕</button>
                </div>
                <div className="p-4">
                ...
                <div className="p-3 border-t border-[#222]">
                  {char.loraUrl ? (
                    <div className="text-[10px] text-green-400 text-center">✅ LoRA 已訓練</div>
                  ) : (
                    <label className="w-full py-2 border border-[#333] rounded-lg text-[10px] font-bold tracking-widest uppercase text-[#555] hover:border-[#e8d5b0] hover:text-[#e8d5b0] transition-all cursor-pointer flex items-center justify-center gap-1">
                      🧠 訓練 LoRA
                      <input type="file" accept=".zip" className="hidden" onChange={(e) => handleTrainLora(char.id, e)} />
                    </label>
                  )}
                </div>
              </div>
